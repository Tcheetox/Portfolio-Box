...
{
	public class FileController(ILogger<FileController> logger, User user, ISharedFileRepository sharedFileRepository, ISharedFileFactory sharedFileFactory) : Controller
	{
		private readonly ILogger<FileController> _logger = logger;
		private readonly ISharedFileRepository _sharedFileRepository = sharedFileRepository;
		private readonly ISharedFileFactory _sharedFileFactory = sharedFileFactory;
		...
		public IActionResult DownloadById(int id)
		{
			...
			return file is null ? NotFound() : PhysicalFile(file.DiskPath, MediaTypeNames.Application.Octet, file.OriginalName);
		}
		...
		public IActionResult Download(string id)
		{
			...
			return file is null
				? View("FileNotFound", new FileNotFoundModel(_user))
				: PhysicalFile(file.DiskPath, MediaTypeNames.Application.Octet, file.OriginalName);
		}
		...
		public async Task<IActionResult> Upload()
		{
			...
			return uploadedFiles.Count != 0
				? Created(nameof(FileController), uploadedFiles)
